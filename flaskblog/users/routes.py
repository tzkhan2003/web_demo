...
from flaskblog.models import User, Post, Comment, Blog
...
@users.route("/user/<string:username>/blog")
@login_required
def user_blogs(username):
    ...
    page = request.args.get('page', 1, type=int)
    user = User.query.filter_by(username=username).first_or_404()
    posts = Blog.query.filter_by(author7=user)\
        .order_by(Blog.date_posted.desc())\
        .paginate(page=page, per_page=5)
    return render_template('user_blogs.html', posts=posts, user=user,title='Blog')
